...
namespace XLua.CSObjectWrap
{
    public class Config_SkillBridge : LuaBase, Config_Skill
    {
        ...
        TimerContinueType Config_Skill.timerContinueType 
        {
            ...
            get 
            {
                ...
                lock (luaEnv.luaEnvLock)
                {
					...
					ObjectTranslator translator = luaEnv.translator;
					LuaAPI.lua_getref(L, luaReference);
					LuaAPI.xlua_pushasciistring(L, "timerContinueType");
					if (0 != LuaAPI.xlua_pgettable(L, -2))
					{
						luaEnv.ThrowExceptionFromError(oldTop);
					}
					TimerContinueType __gen_ret;translator.Get(L, -1, out __gen_ret);
					LuaAPI.lua_pop(L, 2);
					return __gen_ret;
#if THREAD_SAFE || HOTFIX_ENABLE
                }
